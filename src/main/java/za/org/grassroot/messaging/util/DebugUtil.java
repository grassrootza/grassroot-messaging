...
public class DebugUtil {
    ...
    public static void transactionRequired(Class<?> clazz) {
        if (!transactionDebugging) {
            return;
        }

        if (verboseTransactionDebugging) {
            showTransactionStatus(clazz.getSimpleName());
        }

        if (!transactionActive()) {
            throw new IllegalStateException("Transaction required but not active [" + clazz.getSimpleName() + "]");
        }
    }
}
